...
from Models.models_utils import ResDownBlock
...
from helpers import printProgressBar, get_labels
...
def get_R_model(k, type):
    ...
    if type == 'ScrabbleGAN':
        img_org = Input(shape=(None, 32, 1))
        img_shifted = Input(shape=(None, 32, 1))
        ...
        img_org = Input(shape=(28, 28, 1))
        img_shifted = Input(shape=(28, 28, 1))

        images = Concatenate()([img_org, img_shifted])

        images = Conv2D(64, 5, 2, padding='same')(images)
        images = BatchNormalization()(images)
        images = ReLU()(images)
        images = MaxPooling2D()(images)

        images = Conv2D(128, 5, 2, padding='same')(images)
        images = BatchNormalization()(images)
        images = ReLU()(images)
        images = MaxPooling2D()(images)
...
class GanLatentDiscover:
    def __init__(self, G_model, gen_type, k):
        ...
        if gen_type == 'GAN':
            self.d = self.G_model.input_shape[1]
        elif gen_type == 'CGAN':
            self.d = self.G_model.input_shape[0][1]
        elif gen_type == 'ScrabbleGAN':
            self.d = self.G_model.input_shape[1][1]
    ...
    def run_g_forward(self, seeds, labels):
        if self.gen_type == 'GAN':
            images_org = self.G_model(seeds).numpy()

        elif self.gen_type == 'CGAN':
            images_org = self.G_model([seeds, labels]).numpy()

        elif self.gen_type == 'ScrabbleGAN':
            images_org = self.G_model([labels, seeds]).numpy()
    ...
    def train(self, path, epochs, steps_per_epoch, batch_size):
        ...
        def train_c_step(noise, labels, ks, eps, k_vectors):
            with tf.GradientTape() as r_tape, tf.GradientTape() as a_tape:
                ...
                if self.gen_type == 'CGAN':
                    img_org, img_shifted = self.G_model([noise, labels]), self.G_model([shifted_noises, labels])
                if self.gen_type == 'ScrabbleGAN':
                    img_org, img_shifted = self.G_model([labels, noise]), self.G_model([labels, shifted_noises])
        ...
        @tf.function
        def train_step(noise, ks, eps, k_vectors):
            with tf.GradientTape() as r_tape, tf.GradientTape() as a_tape:

                shifted_noises = self.A_model([k_vectors, noise])
                img_org, img_shifted = self.G_model(noise), self.G_model(shifted_noises)
                est_k, est_eps = self.R_model([img_org, img_shifted])
        ...
        fixed_labels = get_labels(is_random=False, n_samples=10, type=self.gen_type)
        ...
        for epoch in range(epochs):
            ...
            for step in range(steps_per_epoch):
                ...
                if self.gen_type == 'GAN':
                    k_loss_step, eps_error_step = train_step(train_noise, train_ks, train_eps, train_k_vectors)
                else:
                    labels = get_labels(is_random=True, n_samples=batch_size, type=self.gen_type)
            ...
            if epoch > 50 and (epoch % 10 == 0):
                ...
                plot_images(org_images, shifted, epoch, fixed_eps, os.path.join(path, 'images'), transpose=self.gen_type=='ScrabbleGAN')
